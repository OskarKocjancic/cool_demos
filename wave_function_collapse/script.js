...
class Cell {
  ...
  draw(entropy) {
    ...
    img.onload = () => {
      ...
      if (entropy)
        ctx.fillText(
          this.availableCellTypes.length,
          this.x * cellSize,
  ...
  updateSurroundingCells() {
    ...
    positions.forEach((p) => {
      try {
        ...
        if (
          x + p[0] < grid.length &&
          y + p[1] < grid[x + p[0]].length &&
          neighboringCell.availableCellTypes.length != 0
        ) {
          toRemove[k].forEach((c) => {
            if (neighboringCell.availableCellTypes.indexOf(c) != -1) {
              neighboringCell.availableCellTypes.splice(
                neighboringCell.availableCellTypes.indexOf(c),
                1
              );
            }
          });
...
function findLowestEntropy() {
  ...
  for (let i = 0; i < grid.length; i++) {
    for (let j = 0; j < grid[i].length; j++) {
      min =
        grid[i][j].availableCellTypes.length < min &&
        grid[i][j].availableCellTypes.length != 0
          ? grid[i][j].availableCellTypes.length
          : min;
    }
  }
  for (let i = 0; i < grid.length; i++) {
    for (let j = 0; j < grid[i].length; j++) {
      if (grid[i][j].availableCellTypes.length == min)
        arrOfLowestEntropies.push(grid[i][j]);
    }
  }
  return arrOfLowestEntropies[
    Math.floor(Math.random() * arrOfLowestEntropies.length)
  ];
}
...
function updateCellWithLowestEntropy() {
  ...
  if (cell != undefined) {
    cell.changeCellType(
      cell.availableCellTypes[
        Math.floor(Math.random() * cell.availableCellTypes.length)
      ]
    );
